...
p2_b <- ggplot(df_p2_b) +
  ...
  theme(
    ...
    axis.title.y.left = element_text(color = "#E41A1C"),             # "black"#E41A1C
...
There is an error in previous formula, we remodel panel c
...
# Here we use the spline function to decompose the impact of
# falling fine particles on temperature differences.
...
panel_group <- panel_million_mob %>%
  left_join(centers_group, by = "ID") %>%
  mutate(pm2p5_mera = 10^9 * pm2p5_mera)                                # ug/m^3
...
# calculate baselines of temperature differences for relaive change.
# across climates and overal mean.
...
fun_spline <- function(
  ...
  yy_tempe <- (
    ...
    as.numeric() + cf_tempe["shutdown_2020"]                         # intercept
  yy_TI <- ((
    as.matrix(ns(pts, knots = kts, Boundary.knots = bd_kts)) %*% 
      as.matrix(cf_TI[paste("ns", 1:nn, sep = "_")])
    ) %>%
      as.numeric() + cf_TI["shutdown_2020"]) %>%                     # intercept
    exp()                                                            # OR
...
boot_spline <- function(
  ...
  mydf <- data.frame(
    ...
    # "na.rm = TRUE" works for climate Tropical, which contain missing values
    mean_TI = apply(
      rawdf_TI[, 2:dim(rawdf_TI)[2]], 1, mean, na.rm = TRUE
    ),
    se_TI = apply(
      rawdf_TI[, 2:dim(rawdf_TI)[2]], 1, sd, na.rm = TRUE
    ),
    CI_low_TI = apply(
      rawdf_TI[, 2:dim(rawdf_TI)[2]], 1,
...
df_new_Boreal <- df_all_Boreal %>%
  mutate(scale_frequency = -110 + 700*frequency, scale_base = -110)

df_new_Dry <- df_all_Dry %>%
  mutate(scale_frequency = -64 + 400*frequency, scale_base = -64)

df_new_Temperate <- df_all_Temperate %>%
  mutate(scale_frequency = -33 + 200*frequency, scale_base = -33)

df_new_Tropical <- df_all_Tropical %>%
  mutate(scale_frequency = -15 + 120*frequency, scale_base = -15)
